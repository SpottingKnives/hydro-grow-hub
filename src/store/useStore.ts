...
export const useStore = create<AppState>()(persist((set, get) => ({
  ...
  deleteParameter: (pid) => set((s) => {
    if (blockPresetDelete('parameter', pid)) return s;
    return { parameters: s.parameters.filter((p) => p.id !== pid), environments: s.environments.map((e) => ({ ...e, parameter_ids: e.parameter_ids.filter((id) => id !== pid) })) };
  }),
  ...
  deleteFeedSchedule: (fid) => set((s) => {
    if (blockPresetDelete('feedSchedule', fid)) return s;
    return { feedSchedules: s.feedSchedules.filter((f) => f.id !== fid) };
  }),
  ...
  deleteNutrient: (nid) => set((s) => {
    if (blockPresetDelete('nutrient', nid)) return s;
    return { nutrients: s.nutrients.filter((n) => n.id !== nid), feedSchedules: s.feedSchedules.map((f) => ({ ...f, rows: f.rows.filter((r) => r.nutrient_id !== nid).map((r, i) => ({ ...r, order_index: i })) })) };
  }),
